...
def forecast_revenue(periods: int = 3):
    ...
    # --- 95% confidence range -----------------------------------------
    # SARIMAX can produce its own intervals, but on a short, lumpy series its
    # internal variance estimate can become unstable and return absurdly wide
    # bands. A more robust, widely used approach: measure how far the model's
    # own fitted values typically miss the actuals (the residual spread), then
    # widen the band for each step further into the future, because error
    # compounds with distance. 1.96 standard deviations covers about 95%.
    resid = np.asarray(model.resid)[13:]          # skip start-up months
    sigma = float(np.std(resid))
    steps = np.arange(1, periods + 1)
    half_width = 1.96 * sigma * np.sqrt(steps)
    conf = np.column_stack([point - half_width, point + half_width])
